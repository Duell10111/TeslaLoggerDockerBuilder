...
LOCAL_WEBSERVER_IMAGE="teslalogger_webserver:latest"
LOCAL_TESLALOGGER_IMAGE="teslalogger_teslalogger:latest"
...
if ! docker manifest inspect "$REGISTRY/$IMAGE_PREFIX/$WEB_SERVER_IMAGE_NAME:$LATEST_TAG" || ! docker manifest inspect "$REGISTRY/$IMAGE_PREFIX/$TESLA_LOGGER_IMAGE_NAME:$LATEST_TAG"; then
  ...
  echo "Beide Befehle waren erfolgreich (Exit-Code 0)."
  echo "Images already existing"
  exit 1
fi
